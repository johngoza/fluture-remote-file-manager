...
const {def, ConnectionConfig, FtpClientType, ReadStreamType, WriteStreamType} = require("./sanctuary-environment");
...
const getFileViaFtp =
  def("getFileViaFtp")
  ({})
  ([FtpClientType, WriteStreamType, ConnectionConfig, FutureType($.String)($.Void)])
  (ftpClient => writeStream => connectionConfig => Future((reject, resolve) => {
    ...
    ftpClient.on("ready", function() {
      ftpClient.get(remotePath, function(err, stream) {
        ...
        return resolve(stream.pipe(writeStream));
      });
    });
